...
export const getPostLikeByID = async (db, id) => {
    const [data] = await db.query('SELECT COUNT(id) AS count FROM likes WHERE post_id=?', [id])
    return data
}

export const removeLike = async (db, id) => {

    return await db.query('DELETE from likes where id=? limit 1', [id], { return: true })
}

export const addLike = async (db, body, user) => {

    return await db.query('INSERT INTO likes values (?,?,?,?)', [uuidv4(), user.id, body.post_id, new Date()], { return: true })
}
export const getUserPostLikeByID = async (db, postId, userId) => {
    const [data] = await db.query('SELECT * FROM likes WHERE post_id=? and user_id = ?', [postId, userId], { return: true })
    return data
}
